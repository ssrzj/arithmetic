...
public class RadarView extends View {
    ...
    private List<RadarData> secondDataList;
    private int count = 4;//雷达网圈数
    ...
    private float textSize = 10f;//字体大小sp
    ...
    private void drawRegion2(Canvas canvas) {
        ...
        for (int i = 0; i < count; i++) {
            double percent = secondDataList.get(i).getPercentage()/ maxValue;
    ...
    private void drawText(Canvas canvas) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            String percentage = secondDataList.get(i).getPercentage()+"/"+dataList.get(i).getPercentage();
            float dis = textPaint.measureText(title);//文本长度
            canvas.drawText(title, x - dis / 2, y, textPaint);
            canvas.drawText(percentage,x - dis /2, y+dis/2,textPaint);
        }
    }
...
//        return finalData!=null&&finalData.data.get(0).size()>=3;
    }
    ...
    public void setText(ArrayList titles) {
        this.titles = titles;
    }

    public void setSecondDataList(List<RadarData> secondDataList) {
        this.secondDataList = secondDataList;
    }


    public void updateFirst(int progress) {
        for(int i=0;i<dataList.size();i++){
            dataList.get(i).setPercentage(progress);
        }
        invalidate();
    }
    public void updateSecond(int progress){
        for(int i=0;i<secondDataList.size();i++){
            secondDataList.get(i).setPercentage(progress);
        }
        invalidate();
    }
}
